...
def draw_triangle_classification(ax, problem_id, problem):
    ...
    if problem_id == 'G021':
        ...
        pts = np.array([(1, 1), (5, 1), (3, 1 + 4*np.sin(np.radians(60)))])
        triangle = Polygon(pts, closed=True, facecolor=COLORS['fill'],
                          edgecolor=COLORS['line'], linewidth=2)
        ax.add_patch(triangle)

        # Add side labels
        ax.text(3, 0.6, "5 cm", ha='center', fontsize=10, color=COLORS['text'])
        ax.text(4.2, 3, "5 cm", ha='left', fontsize=10, color=COLORS['text'])
        ax.text(1.8, 3, "5 cm", ha='right', fontsize=10, color=COLORS['text'])

        # Add congruency marks (one tick per side)
        # Bottom side (Perpendicular to 0° is 90°)
        ax.plot([3, 3], [0.85, 1.15], color=COLORS['line'], linewidth=1.5)
        # Right side (Perpendicular to 120° is 210°)
        x_mid, y_mid = 4, 1 + 2*np.sin(np.radians(60))
        angle = np.radians(120 + 90)
        dx, dy = 0.15*np.cos(angle), 0.15*np.sin(angle)
        ax.plot([x_mid - dx, x_mid + dx], [y_mid - dy, y_mid + dy], color=COLORS['line'], linewidth=1.5)
        # Left side (Perpendicular to 60° is 150°)
        x_mid, y_mid = 2, 1 + 2*np.sin(np.radians(60))
        angle = np.radians(60 + 90)
        dx, dy = 0.15*np.cos(angle), 0.15*np.sin(angle)
        ax.plot([x_mid - dx, x_mid + dx], [y_mid - dy, y_mid + dy], color=COLORS['line'], linewidth=1.5)

    elif problem_id == 'G022':
        # Right-isosceles triangle (angles 45, 45, 90)
        # Legs are equal (length 4)
        pts = np.array([(1, 1), (5, 1), (1, 5)])
        triangle = Polygon(pts, closed=True, facecolor=COLORS['fill'],
                          edgecolor=COLORS['line'], linewidth=2)
        ax.add_patch(triangle)

        # Right angle marker (Square at 1,1)
        ax.plot([1, 1.4, 1.4], [1.4, 1.4, 1], color=COLORS['text'], linewidth=1.5)

        # Angle labels
        ax.text(1.6, 1.6, "90°", ha='center', fontsize=9, color=COLORS['text'], weight='bold')
        ax.text(3.5, 1.3, "45°", ha='center', fontsize=9, color=COLORS['text'])
        ax.text(1.3, 4, "45°", ha='center', fontsize=9, color=COLORS['text'])
...
def draw_pie_chart(ax, problem_id, problem):
    ...
    # If it's a 90 degree sector, add a right angle square marker at the center
    if angle == 90:
        square_size = 0.15
        ax.plot([0, square_size, square_size], [square_size, square_size, 0], 
                color=COLORS['text'], linewidth=1.5)

    # NO FRACTION LABEL (Leaks answer)
