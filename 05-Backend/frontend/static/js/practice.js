...
class Practice {
        ...
        if (hintBtn) {
            hintBtn.addEventListener('click', async () => {
                try {
                    ...
                    const response = await fetch(`http://localhost:50519/api/v1/problems/${problemId}/hint`);
        ...
        if (exampleBtn) {
            exampleBtn.addEventListener('click', async () => {
                try {
                    ...
                    const response = await fetch(`http://localhost:50519/api/v1/pathways/${pathwayType}/example`);
    ...
    async loadGlossary() {
        try {
            const response = await fetch('http://localhost:50519/api/v1/glossary');
